...
export const VideoPreview = forwardRef<HTMLVideoElement, Props>(function VideoPreview(
  ...
  const [canShowVideo, setCanShowVideo] = useState<boolean>(() => !shouldDefer);
  ...
  const needsAbsoluteWrapper = /\b(absolute|fixed|sticky)\b/.test(classNameSafe);
  ...
  const internalTransitionClass =
    !canShowVideo || useInternalOpacity ? "transition-opacity duration-300" : "";
  const combinedClassName =
    `${classNameSafe} ${internalOpacityClass} ${internalTransitionClass}`.trim();

  return (
    <div className={wrapperClassName}>
      {!canShowVideo && <div className={`absolute inset-0 ${placeholderClassName}`} aria-hidden />}
      <OptimizedVideo
        ref={ref}
        src={src}
